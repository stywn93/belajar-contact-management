...
import {contactUpdate, contactDetail} from "../../lib/api/ContactApi.js";
...
export default function ContactEdit() {
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        const response = await contactUpdate(token, {id, first_name:first_name, last_name:last_name, email:email, phone:phone});
        const responseBody = await response.json();

        if(response.status === 200){
            await alertSuccess("Berhasil update contact");
    ...
    useEffectOnce(() => {
            getContactDetail()
                .then(() => console.log("berhasil fetch"))
        }
    )
